document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0, len = squares.length; i < len; i++)
    ...
    (function(index) {
      ...
      squares[i].onclick = function() {
        ...
        if(squares[index + 7].classList.contains('taken')) {
          ...
          squares[index].classList.add('chosen')
          squares[index].classList.add('taken')
          
        } else alert('That spot is unavailable.')
      }
    })(i)
    ...
    function checkBoard() {
      ...
      //now take the 4 values in each member of winningArrays and plug them into our grid squares
      for(let y=0; y<winningArrays.length; y++) {
        ...
        //now check those arrays in our own grid to see if they all have the class of player-one
        if (square1.classList.contains('player-one') &&
        square2.classList.contains('player-one') && 
        square3.classList.contains('player-one') && 
        square4.classList.contains('player-one')) {
          //if they do, player-one is passed as the winner!
          result.innerHTML = 'Player One Wins! (To play again, reload the page.)'
          result.classList.remove('hide');
          result.classList.add('victory');
          //since result is determined, make all squares unclickable (.taken has "pointer-events: none")
        ...
        square4.classList.contains('player-two')) {
          result.innerHTML = 'Player Two Wins! (To play again, reload the page.)'
          result.classList.remove('hide');
          result.classList.add('victory');
          squares.forEach(square => square.classList.add('taken'))
        }
      }
      let playableSquares = document.querySelectorAll('.playable');
      playableSquares = Array.from(playableSquares);
      if (playableSquares.every(square => square.classList.contains('chosen'))) {
        result.classList.remove('hide');
        result.innerHTML = "No winner! Reload to start a new game."
